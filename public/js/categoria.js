...
const initCategoriaTable = async () => {
    categoriaDT = $("#tableCategoria").DataTable({
        order: [],
        searching: false,
        responsive: true,
        theme: "base",
...
nuevoBtn.addEventListener("click", async function () {
    try {
        const url = route("categoria.create");
...
const registrar = async () => {
    ...
    // URL para registrar
    const url = route("categoria.store");
...
const editar = async (id) => {
    ...
    try {
        // Asignamos la ruta
        const url = route("categoria.edit", id);
...
const update = async (id) => {
    ...
    // URL para registrar
    const url = route("categoria.update", id);
    // Tomar el formulario de registro por si id
    const form = document.getElementById("editFormCategoria");
    // Crear un objeto formData
    const formData = new FormData(form);
    // Indicar el metodo a usar
...
const eliminar = async (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const url = route("categoria.destroy", id);
